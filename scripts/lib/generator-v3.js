...
5. DIAGRAMS: You must include exactly 1 Mermaid.js diagram every 300-400 words (approx. 2 diagrams total). Wrap them in: <pre class="mermaid"> graph TD; ... </pre>. Ensure they accurately map to the architecture. MAKE THEM VISUALLY APPEALING: Use specific node shapes like '[(Database)]', '((Circle))', '{Decision}' instead of basic boxes, and embed FontAwesome icons inside the nodes (e.g., "A[<i class='fas fa-server'></i> Server]").
